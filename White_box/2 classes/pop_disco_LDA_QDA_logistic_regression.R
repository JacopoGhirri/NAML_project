...
data<-rbind(pop_data,disco_data)
...
glm_model_1<-glm(data$binary_genre~ data$zcr+data$rms_energy+data$mean_chroma+data$spec_flat+data$hf_contrast+data$lf_contrast ,family=binomial( link = logit ))
summary(glm_model_1)



anova( glm_model_1, glm_model, test = "Chisq" )
#if low pvalue recuced model is less significant

#table
threshold = 0.5
real  = data$binary_genre
predicted = as.numeric( glm_model_1$fitted.values > threshold )
