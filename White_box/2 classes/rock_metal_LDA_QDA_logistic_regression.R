...
data<-rbind(rock_data,metal_data)
...
glm_model<-glm(data$binary_genre~ data$zcr+data$rms_energy+data$mean_chroma+data$spec_flat+data$hf_contrast+data$mf_contrast ,family=binomial( link = logit ))
summary(glm_model)

glm_model<-glm(data$binary_genre~ data$zcr+data$rms_energy+data$mean_chroma+data$spec_flat+data$hf_contrast ,family=binomial( link = logit ))
summary(glm_model)

glm_model<-glm(data$binary_genre~ data$zcr+data$rms_energy+data$mean_chroma+data$spec_flat ,family=binomial( link = logit ))
summary(glm_model)

glm_model_4<-glm(data$binary_genre~ data$zcr+data$mean_chroma+data$spec_flat ,family=binomial( link = logit ))
summary(glm_model_4)
